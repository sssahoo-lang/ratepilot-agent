...
function exportNegotiationPDF(negotiation) {
  ...
  doc.setFontSize(18);
  doc.setTextColor(20, 20, 20);
  doc.text("RatePilot — Negotiation Summary", margin, y);
  y += 9;
  ...
  for (let page = 1; page <= pages; page += 1) {
    ...
    doc.text(`Generated by RatePilot • ratepilott.netlify.app • ${today}`, margin, 287);
  }

  doc.save(`ratepilot-${providerSlug}-${negotiation.id}.pdf`);
}
...
function SavingsOverview({ negotiations }) {
  ...
  const monthlySavings = wonNegotiations.reduce((sum, n) => sum + (Number(n.monthly_savings) || 0), 0);
  ...
  const sortedRows = [...negotiations].sort((a, b) => (Number(b.monthly_savings) || 0) - (Number(a.monthly_savings) || 0));
    ...
    <section className="savings-overview">
      ...
      <div className="savings-table-wrap">
        <table className="savings-table">
          ...
          <tbody>
            {sortedRows.map((n) => {
              const savings = Number(n.monthly_savings) || 0;
  ...
  return (
    <div className="step-item">
      ...
      <div className="step-content">
        ...
        {step.step_type === "summary" && content.summary && (
          <p className="step-body-text" style={{ marginTop: 6, lineHeight: 1.6, color: "var(--text-secondary)" }}>{content.summary}</p>
        )}
...
const UPLOAD_QUOTES = [
  { text: "The worst they can say is no. We write the ask.", attr: "RatePilot" },
  { text: "Most people leave $400/year on the table. Not anymore.", attr: "RatePilot" },
  { text: "Your provider is counting on you not to call. We do it for you.", attr: "RatePilot" },
  { text: "Negotiation used to be a skill. Now it's automated.", attr: "RatePilot" },
  { text: "73% of customers who ask for a lower rate — get one.", attr: "Consumer Reports" },
];
...
function UploadView({ onDone, showToast }) {
  ...
  useEffect(() => {
    const tick = setInterval(() => {
      setQuoteFade(false);
      setTimeout(() => {
        setQuoteIdx((i) => (i + 1) % UPLOAD_QUOTES.length);
        setQuoteFade(true);
      }, 400);
    }, 4500);
    ...
    <div className="upload-grid">
      ...
      <div className="upload-tips">
        ...
        <div style={{
          ...
          <p style={{ margin: 0, fontSize: 13, fontStyle: "italic", color: "var(--text-primary)", lineHeight: 1.6 }}>
            "{UPLOAD_QUOTES[quoteIdx].text}"
          </p>
          <span style={{ marginTop: 6, fontSize: 11, color: "var(--text-muted)", letterSpacing: "0.04em" }}>
            — {UPLOAD_QUOTES[quoteIdx].attr}
          </span>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 20 }}>
          {[
            { value: "$420", label: "avg saved per bill / year" },
            { value: "73%", label: "of askers get a lower rate" },
            { value: "< 5 min", label: "to launch your first negotiation" },
          ].map(({ value, label }) => (
            <div key={label} style={{
              background: "var(--bg-surface)",
              border: "1px solid var(--bg-border)",
              ...
              <div style={{ fontFamily: "var(--font-mono)", fontWeight: 700, fontSize: 15, color: "var(--brand)", marginBottom: 4 }}>{value}</div>
              <div style={{ fontSize: 10, color: "var(--text-muted)", lineHeight: 1.4 }}>{label}</div>
            </div>
          ))}
        </div>

        {/* How it works steps */}
        <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.07em", color: "var(--text-muted)", textTransform: "uppercase", marginBottom: 10 }}>How it works</p>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {[
            { n: "01", title: "Upload your bill", body: "PDF, photo, or plain text — AI extracts provider, amount, and account details." },
            { n: "02", title: "Agent builds strategy", body: "Estimates competitor rates and sets your target price and walkaway threshold." },
            { n: "03", title: "Draft goes out", body: "A firm, personalized negotiation email lands in your drafts, ready to send." },
            { n: "04", title: "Paste their reply", body: "Agent reads the response and decides: accept, counter, or escalate for you to review." },
          ].map(({ n, title, body }) => (
            <div key={n} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
              ...
              <div>
                ...
                <p style={{ margin: "2px 0 0", fontSize: 11, color: "var(--text-muted)", lineHeight: 1.5 }}>{body}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ marginTop: 18, paddingTop: 14, borderTop: "1px solid var(--bg-border)", display: "flex", flexWrap: "wrap", gap: 6 }}>
          {["PDF", "TXT", "JPG", "PNG", "WEBP", "GIF"].map((fmt) => (
            <span key={fmt} style={{
          ...
          <span style={{ fontSize: 10, color: "var(--text-muted)", alignSelf: "center" }}>accepted</span>
        </div>
      </div>
    </div>
  );
...
function NegotiationOutcomeCard({ negotiation }) {
  ...
  const savings = Number(negotiation.monthly_savings) || 0;
  const dealClosed = negotiation.status === "won" && savings > 0;
  const hasOffer = best !== null;
  const savingsMonth = Number(negotiation.monthly_savings) || 0;
  const savingsYear = Number(negotiation.annual_savings) || 0;
  const savingsPct = original > 0 && savingsMonth > 0 ? Math.round((savingsMonth / original) * 100) : 0;
  ...
  return (
    <div style={{ background: "var(--bg-surface)", border: "1px solid var(--bg-border)", borderRadius: 12, padding: "18px 20px", marginTop: 24 }}>
      ...
      {dealClosed && (
        <div style={{ background: "color-mix(in srgb, var(--green-dim) 45%, transparent)", border: "1px solid var(--green-dim)", borderRadius: 8, padding: "10px 14px", fontSize: 13, color: "var(--green)", lineHeight: 1.5 }}>
          ✅ Deal closed — saving <strong>{fmt(savings)}/mo</strong> · <strong>{fmt(savingsYear)}/year</strong>.
        </div>
      )}
      {!hasOffer && (
